...
class Plotter:  
    ...
    def plot_vision_odometry(self, limits = (11, 16.5)):
        ...
        angles.set(xlabel='time steps', ylabel='angular difference', title='Angular error from Ground Truth to Odometry on time')
        ...
        angles.set(xlabel='time steps', ylabel='w (degrees)', title='Ground Truth and Odometry angles on time')
        angles.legend(loc='best')
        ...
        errors.set(xlabel='time', ylabel='distance in meters', title='Euclidian distance from Ground Truth to Odometry on time')
    ...
    def plot_vision_mcl(self, limits = (11, 16.5)):
        figure, position = plt.subplots()
        position.plot(self.file.get_vision()[:, 0], self.file.get_vision()[:, 1], 'r', label="ground truth")
        position.plot(self.file.get_mcl()[:, 0], self.file.get_mcl()[:, 1], 'b', linestyle='--', label="monte carlo localization")
        position.set(xlabel='x (m)', ylabel='y (m)', title='Ground Truth and Monte Carlo Localization positions')
        position.legend(loc='best')
        path = self.get_graph_path("graphs_split", "mcl_positions")
        figure.savefig(path, dpi=500)

        figure, angles = plt.subplots()
        angles.plot(self.file.get_times(), self.get_mcl_angle_errors(), 'b', label="monte carlo localization", linestyle='--')
        angles.set(xlabel='time steps', ylabel='angular difference', title='Angular error from Ground Truth to Monte Carlo Localization on time')
        angles.legend(loc='best')
        ...
        figure, angles = plt.subplots()
        angles.plot(self.file.get_times(), self.file.get_vision()[:, 2], 'r', label="ground truth")
        angles.plot(self.file.get_times(), self.file.get_mcl()[:, 2], 'b', label="monte carlo localization", linestyle='--')
        angles.set(xlabel='time steps', ylabel='w (degrees)', title='Ground Truth and Monte Carlo Localization angles on time')
        angles.legend(loc='best')
        ...
        errors.set(xlabel='time', ylabel='distance in meters', title='Euclidian distance from Ground Truth to Monte Carlo Localization on time')
    ...
    def plot_vision_odometry_mcl(self, limits = (11, 16.5)):
        figure, position = plt.subplots()
        position.plot(self.file.get_vision()[:, 0], self.file.get_vision()[:, 1], 'r', label="ground truth")
        position.plot(self.file.get_odometry()[:, 0], self.file.get_odometry()[:, 1], 'g', linestyle='--', label="odometry")
        position.plot(self.file.get_mcl()[:, 0], self.file.get_mcl()[:, 1], 'b', linestyle='--', label="monte carlo localization")
        position.set(xlabel='x (m)', ylabel='y (m)', title='Ground Truth, Odometry and Monte Carlo Localization positions')
        position.legend(loc='best')	
        ...
        angles.plot(self.file.get_times(), self.get_odm_angle_errors(), 'g', label="odometry", linestyle='--')
        angles.plot(self.file.get_times(), self.get_mcl_angle_errors(), 'b', label="monte carlo localization", linestyle='--')
        angles.set(xlabel='time steps', ylabel='angular difference', title='Angular error from Ground Truth to Odometry and Monte Carlo Localization on time')
        angles.legend(loc='best')
        ...
        angles.plot(self.file.get_times(), self.file.get_odometry()[:, 2], 'g', label="odometry", linestyle='--')
        angles.plot(self.file.get_times(), self.file.get_mcl()[:, 2], 'b', label="monte carlo localization", linestyle='--')
        angles.set(xlabel='time steps', ylabel='w (degrees)', title='Ground Truth, Odometry and Monte Carlo Localization angles on time')
        angles.legend(loc='best')
        ...
        errors.plot(self.file.get_times(), self.get_mcl_errors(), 'b', label="monte carlo localization distance")
        errors.set(xlabel='time', ylabel='distance in meters', title='Distance from Ground Truth to Odometry and to Monte Carlo Localization positions on time')
        errors.legend(loc='best')
